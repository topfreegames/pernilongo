...
PernilongoService.prototype.setupMongoClient = function (){
  ...
  MongoClient.connect(this.mongoUrl, function(err, db) {
    ...
    //creates indexes to avoid duplicates in register & authorize functions
    self.acl.ensureIndexAsync({username: 1, subscribe: 1}, {unique: true});
...
PernilongoService.prototype.authorizePlayerInRoomsWithExpireTime = function (user, rooms, expireTime){
  return this.acl.insertManyAsync(rooms.map(function(topic) {
    return {username: user, subscribe: topic, expires: new Date(Date.now() + 1000*expireTime)};
  }));
}
