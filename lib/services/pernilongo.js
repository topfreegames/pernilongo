...
PernilongoService.prototype.setupMongoClient = function (){
  ...
  MongoClient.connect(this.mongoUrl, function(err, db) {
    ...
    //creates index to expire authorization
    self.acl.createIndexAsync({"expires": 1 }, { expireAfterSeconds: 0 });
    
    //creates index to avoid duplicates in registration
    self.users.createIndexAsync({username: 1}, {unique: true});
  })
...
PernilongoService.prototype.authorizePlayerInRooms = function (user, rooms){
  var bulk = this.acl.initializeUnorderedBulkOp({useLegacyOps: true});
  rooms.forEach(function(topic) {
    bulk.find({username: user, subscribe: topic}).upsert().update({$set: {username: user, subscribe: [topic]}});
  })
  return bulk.execute();
}
...
PernilongoService.prototype.authorizePlayerInRoomsWithExpireTime = function (user, rooms, expireTime){
  var bulk = this.acl.initializeUnorderedBulkOp({useLegacyOps: true});
  rooms.forEach(function(topic) {
    bulk.find({username: user, subscribe: topic}).upsert().update({$set: {username: user, subscribe: [topic], expires: new Date(Date.now() + 1000*expireTime)}});
  })
  return bulk.execute();
}
